    ...
    <v-form v-model="valid" class="subjectForm">
      <v-container>
        <v-row v-for="(item, index) in subjectList" :key="index">
          ...
          <v-col>
            <v-container>
              <v-icon v-if="item.errors == true" color="red">
                mdi-alert
              </v-icon>
              <v-icon v-if="item.errors == false" color="green">
                mdi-check-bold
              </v-icon>  
            </v-container>
          </v-col>
  ...
  methods: {
    ...
    async submit() {
      ...
      for (let i=0; i<out.length; i++) {
        try {
          console.log(out[i]);
          let res = await subjectAPI.createSubject(out[i].name, out[i].subjectId, out[i].description);
          console.log(res);
